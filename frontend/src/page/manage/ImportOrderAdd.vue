        ...
        <div v-if="items.length > 0" class="items-list">
            <div v-for="(group, gi) in groupedItems" :key="gi" class="card product-group-card">
                ...
                <table class="data-table sku-table">
                    ...
                    <tbody>
                    <tr v-for="item in group.skus" :key="item.skuId">
                        <td>{{ item.skuName || '-' }}</td>
                        <td>{{ formatSpec(item.spec) }}</td>
                        <td>¥{{ item.importPrice ?? '0.00' }}</td>
                        <td class="qty-cell">
                            <input v-model.number="item.quantity" class="qty-input" min="1" type="number"/>
                        </td>
                        <td><strong>¥{{ (Number(item.importPrice || 0) * Number(item.quantity || 0)).toFixed(2) }}</strong></td>
                        <td>
                            <button class="btn-danger btn-sm" @click="removeSku(gi, item.skuId)">移除</button>
                        </td>
                    </tr>
                    </tbody>
                </table>
...
function removeSku(groupIndex, skuId) {
    const group = groupedItems.value[groupIndex]
    if (!group) return
    const idx = items.value.findIndex(item => item.skuId === skuId)
    if (idx !== -1) {
